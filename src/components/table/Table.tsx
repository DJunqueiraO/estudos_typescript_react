...
export function Table(props: TableProps) {
  ...
  const on_tfoot_td = props.on_tfoot_td || (
    (column: React.ReactNode, index: number) => (
      <Td className='tfoot-td' key={index}>{column}</Td>
    )
  )

  const on_tfoot_tr = props.on_tfoot_tr || (
    (line: Array<React.ReactNode>, index: number) => (
      <Tr className='tfoot-tr' key={index}>{line.map(on_tfoot_td)}</Tr>
    )
  )

  const on_tbody_td = props.on_tbody_td || (
    (column: React.ReactNode, index: number) => (
      <Td className='tbody-td' key={index}>{column}</Td>
    )
  )

  const on_tbody_tr = props.on_tbody_tr || (
    (line: Array<React.ReactNode>, index: number) => (
      <Tr className='tbody-tr' key={index}>{line.map(on_tbody_td)}</Tr>
    )
  )

  const on_thead_th = props.on_thead_th || (
    (column: React.ReactNode, index: number) => (
      <Th className='thead-th' key={index}>{column}</Th>
    )
  )

  const on_thead_tr = props.on_thead_tr || (
      (line: Array<React.ReactNode>, index: number) => (
    <Tr className='thead-tr' key={index}>{line?.map(on_thead_th)}</Tr>
  )
  )
